...
- 任意で LED の状態全体を `zmk-feature-custom-settings` を通じて公開し、client が
  専用の画面なしで編集できるようにします。BLE のキー操作は設定に即時反映し、
  USB の連続操作は LED を即時反映しつつ設定通知をまとめます。
...
**キー操作は LED に即時反映します。** BLE では使用中の設定も即時に書き換えます。
USB では連続操作をまとめ、最後の操作から 300 ms 後に設定へ書き込みます。
Studio にはその最終値が短い遅延の後に届き、USB の通知キューが連打で満杯に
なるのを防ぎます。USB の Studio 用 CDC ポートが閉じている間は、LED 由来の
設定書き込みに限り、既存の custom-settings API で Studio 通知を抑えます。
値の更新と flash 保存は続き、再接続した client は現在値を読み取れます。
flash への保存は最後のメモリー書き込みから 3 秒後です。
client からの編集は従来どおり即時に反映します。
...
ままであること、ZMK の電池報告が残量 % の取得モードで、リンクされた電池ラッパーを
`chosen` の `zmk,battery` として読むことを確認します。

LED 点灯で電池 ADC が低く出る基板向けに、任意の
`zmk,led-battery-adc-offset` センサを追加できます。`source-sensor` に
`zmk,battery-voltage-divider`(`<&vbatt>` など)、`full-duty-offset-mv = <50>` を
設定し、`chosen` の `zmk,battery` をこのセンサへ向けます。サンプルごとに分圧センサの
電圧を読み、ADC が見た直後の各半分の LED 実点灯率に応じて 0～50 mV を加算します。
消灯時は加算しません。ZMK が報告するのは残量 % で、分圧センサはそれを補正前の電圧から
計算するため、このセンサは補正後の電圧から分圧センサと同じ方法で % を計算し直します。
DYA フォークでは同じノードの `mv-to-pct-thresholds` で補間し、しきい値を持たない
upstream ZMK の分圧センサでは ZMK の固定のリチウムイオン直線(3450 mV で 0%、
4200 mV で 100%)を使います。`SENSOR_CHAN_GAUGE_VOLTAGE` と
`SENSOR_CHAN_GAUGE_STATE_OF_CHARGE` の両方を返します。BAS、split peripheral の
残量、ディスプレイのすべてに補正後の値が届きます。補正は分圧抵抗値によるベースの
固定較正に上乗せされます。50 mV は実測値に合わせて変更し、中間の明るさも実機で
確認してください。% の変換は 2 方式とも 0～5 V の全 mV で ZMK 自身のコード(float の補間と
リチウムイオン直線)と一致することを確認し、実際のコールバックは実物同様に gauge 以外の
チャンネルを拒否するスタブに対して
取得失敗・読み出し失敗・0/50/100% の場合を故障注入で試し、CI で行・分岐 100% を
要求します。ハードウェア実測の代わりにはなりません。
...
決定的な 10 万ステップの連続操作も再生し、小さな `usb_pending.h` helper の行・
分岐カバレッジ 100% を CI の条件にします。さらに、このモジュールの C ソースの全関数を
`tests/runtime/` のスタブ付きハーネスに取り出します。18 種類のカーブとその時計、描画、
アイドルと電源断の扱い、状態 API と 3 つの behavior を、BLE あり・なしの単体キーボード、
split の central(表示、接続時の状態交換、ミラー)、split の peripheral(ミラー、応答、
central の活動状態の採用)としてビルドし、設定ブリッジは USB あり・なしで(転送ごとの
設定、USB の書き込みまとめ、再試行付きの遅延フラッシュ保存、Studio のポートが閉じている
間の通知抑制)確認します。CI は取り出した各関数の行・分岐 100% を要求します。devicetree
からのインスタンス生成、relay の登録、設定の宣言は上記のファームウェア fixture で確認します。

split のミラーそのものは、Zephyr の無線シミュレータ BabbleSim 上でエンドツーエンドに
確認します(`bash ./tests/bsim/run-docker.sh`)。`nrf52_bsim` 向けにビルドした本物の
split central と peripheral が BLE で接続・同期し、どちらかの半分のキーによるパターン変更を
peripheral へミラーして応答が返ることを、両半分のログと
`tests/bsim/split-mirror/snapshot.log` の一致で確認します。初回は DYA フォークと
BabbleSim のワークスペースを Docker ボリューム `zmk-bsim-ws` に作ります。リセットは
対象外です(シミュレーション上のデバイスは再起動すると終了し、シミュレーションも終わります)。

電池の補正も同じく `tests/bsim/battery-sag` でエンドツーエンドに確認します。
シミュレータには SAADC がないため、テスト専用モジュールが分圧センサの代わりになり、
その手前を再現します。3825 mV の電池が LED 全点灯時に 50 mV 下がること、実物の
1 M / 510 k の分圧と 12 ビット・ゲイン 1/6 の SAADC、ピン電圧から先は 507 k の
devicetree 値による分圧ドライバと同じ計算です。両半分が本物のラッパーと ZMK の電池報告を
通して毎秒サンプルし、central は LED を起動時のアニメーションから 0・50・100% へ変え、
peripheral はミラーで追従します。スナップショットには、LED につれて動く補正前の読み値と、
動いてはならない ZMK の報告値(各半分自身の値と、central が模擬リンク越しに
peripheral の BAS から読む値)が入ります。central の分圧センサはしきい値なし、
peripheral はしきい値ありなので、1 回で両方の変換を確認します。オフセットを 0 にすると
報告値が LED につれて動き、テストは失敗します。
USB 向けの抑制策が対象にするのは、このモジュールから生じた LED 設定通知です。
DYA 共通の RPC 送信部は変更しないため、別の Studio 通知元まで含めて送信バッファ
満杯時の停止を防ぐ保証はありません。
